...
export function StudioPage() {
  const { camera, microphone, script, setScript, teleprompter, recorder, onProcessedStream, onToggleRecording, processedVideo } = useOutletContext<{ camera: CameraController; microphone: MicrophoneController; script: string; setScript: (script: string) => void; scriptTitle: string; teleprompter: Teleprompter; recorder: Recorder; onProcessedStream: (stream: MediaStream | null, active: boolean) => void; onToggleRecording: () => Promise<void>; processedVideo: { stream: MediaStream | null; active: boolean } }>()
